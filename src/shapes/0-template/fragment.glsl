...
float PrintDigit(in int n, in vec2 p)
{
  ...
  const int lut[10] =
  int[10](480599, 139810, 476951, 476999, 71028, 464711, 464727, 476228, 481111, 481095);
...
vec3 pat2(in vec2 pixel, in vec2 res)
{
  ...
  if (pixel.y<h)
  {
    ...
    if (uv.x>0.0)
    {
      ...
      f = mix(midgrey, f,
      smoothstep(0.1, 0.101,
      length(vec2(uv.x, (fract(3.0*uv.y+0.5)-0.5)/3.0)-vec2(0.5*res.x/res.y, 0.0))));
  ...
  else
  {
    ...
    t += PrintInt((uv-vec2(-0.5, 0.85))*10.0, int(u_resolution.x));
    t += PrintInt((uv-vec2(0.1, 0.85))*10.0, int(u_resolution.y));

    int ideg = int(u_time*60.0);
    int degs = (ideg) % 60;
    int secs = (ideg/60) % 60;
...
vec3 pat3(in vec2 pixel, in vec2 res)
{
  ...
  if ((int(u_time*float(fps))%fps)==id) col = vec3(1.0);
...
{
  float x0 = u_resolution.x*0.0/3.0;
  float x1 = u_resolution.x*1.0/3.0;
  float x2 = u_resolution.x*2.0/3.0;
  float wi = u_resolution.x/3.0;

  vec3 col = pat1(fragCoord-vec2(x0, 0.0), vec2(wi, u_resolution.y));
  if (fragCoord.x>x1) col = pat2(fragCoord-vec2(x1, 0.0), vec2(wi, u_resolution.y));
  if (fragCoord.x>x2) col = pat3(fragCoord-vec2(x2, 0.0), vec2(wi, u_resolution.y));

  col *= smoothstep(2.0, 4.0, mod(fragCoord.x, wi));

  ivec2 p = ivec2(fragCoord);
  ivec2 r = ivec2(u_resolution);
